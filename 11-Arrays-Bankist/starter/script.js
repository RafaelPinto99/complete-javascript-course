...
console.log(x);
// console.log(x.map(() => 5)); // wont do anything
...
const z = Array.from({ length: 7 }, (_, i) => i + 1); // remember _ is a throw away var since we do not need it
console.log(z);
...
const { deposits, withdrawals } = accounts
  ...
  .reduce(
    (sums, cur) => {
      ...
      return sums; // arrow function's body has curly braces therefore need to manually add return
    },
...
// 4.
// this is a title -> This Is a Nice Tittle
...
1. Calculate the dog age in human years using the following formula: if the dog is <= 2 years old, humanAge = 2 * dogAge. If the dog is > 2 years
...
const calcAverageHumanAge = function (ages) {
  ...
  const avgAdultDogs =
    adultDogs.reduce((acc, cur) => acc + cur, 0) / adultHumanAges.length;
...
// BONUS
// const heaviestFetcherWeight = breeds
//   .filter(dog => dog.activities.includes('fetch'))
//   .reduce((max, dog) => (dog.averageWeight > max ? dog.averageWeight : max), 0);

const fetchWeights = breeds
  .filter(dog => dog.activities.includes('fetch'))
  .map(dog => dog.averageWeight); // remember map returns an array
console.log(fetchWeights);
const heaviestFetcherWeight = Math.max(...fetchWeights);

console.log(heaviestFetcherWeight);
*/

/*
// Coding Challenge #5

Julia and Kate are still studying dogs. This time they want to figure out if the dogs in their house are eating too much or too little food.

- Formula for calculating recommended food portion: recommendedFood = weight ** 0.75 * 28. (The result is in grams of food, and the weight needs
  to be in kg)
- Eating too much means the dog's current food portion is larger than the recommended portion, and eating too little is the opposite.
- Eating an okay amount means the dog's current food portion is within a range 10% above and below the recommended portion (see hint).

YOUR TASKS:
1. Loop over the array containing dog objects, and for each dog, calculate the recommended food portion (recFood) and add it to the object as a
   new property. Do NOT create a new array, simply loop over the array (We never did this before, so think about how you can do this without
   creating a new array).
2. Find Sarah's dog and log to the console whether it's eating too much or too little. HINT: Some dogs have multiple users, so you first need to
   find Sarah in the owners array, and so this one is a bit tricky (on purpose) 🤓
3. Create an array containing all owners of dogs who eat too much (ownersTooMuch) and an array with all owners of dogs who eat too little
  (ownersTooLittle).
...
8. Group the dogs into the following 3 groups: 'exact', 'too-much' and 'too-little', based on whether they are eating too much, too little or
   the exact amount of food, based on the recommended food portion.
...
// 4.
console.log(`${ownersTooMuch.join(' and ')} dogs are eating too much`);
console.log(`${ownersTooLittle.join(' and ')} dogs are eating too much`);
...
// 10.
const dogsSorted = dogs.toSorted((a, b) => a.recFood - b.recFood); // ascending order -> if a > b
